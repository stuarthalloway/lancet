...
(deftest test-ant-project
  (let [listeners (.getBuildListeners ant-project)]
    (is (= (count (filter #(= (class %) org.apache.tools.ant.NoBannerLogger)
                          listeners))
           1))))
...
(deftest test-instantiate-task
  ...
  (is (thrown? IllegalArgumentException (instantiate-task ant-project
                                                          "not-a-task-name"))))
...
(deftest test-fileset
  (let [fs (fileset {:dir "test/lancet/test/data"})]
